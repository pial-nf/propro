...
module.exports = {
    ...
    run: async(client, interaction)=> {
        try {
            ...
            interaction.followUp({ embeds: [makeEmbed(index)], components: [row], ephemeral: true })
            ...
            collector.on('collect', async i => {
                if (i.customId === 'next') {
                    index++
                    if (index === array.videos.length) return i.update({ content: "You've Reached The End!", embeds: [], components: [], ephemeral: true })
                    await i.update({ content: `${index}/${array.videos.length}`, embeds: [makeEmbed(index)], components: [row], ephemeral: true });
                }
            });
        } catch (error) {
            interaction.editReply({ content: "No Videos Found! || An error occured in your code please log it to check", ephemeral: true })
        }
    }
} 
