...
// Add this within your `index.js` or where your bot client is defined.
client.on('messageCreate', async (message) => {
  // Check if the message is a DM and the author is not the bot
  if (message.channel.type === 'DM' && !message.author.bot) {
    // Implement your chatbot logic here
    // This can range from simple canned responses to complex NLP-based interactions

    // For instance, sending back the same message as an echo
    const response = `You said: ${message.content}`;

    // Send a reply in the DM
    message.author.send(response).catch((error) => {
      console.error(`Could not send DM to user ${message.author.tag}.\n`, error);
    });
  }
});
client.on("messageCreate", async (message) => {
